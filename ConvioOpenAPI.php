...
class ConvioOpenAPI
{
	 ...
	 * @access public
	 */
	public $login_password = NULL; // NULL
	...
	private function __getPostData()
	{
		...
		$baseData   = http_build_query(array('v'=>$this->v,'api_key'=>$this->api_key,'response_format'=>$response_format,'login_name'=>$this->login_name));
		//'method'=>$this->__method ,'login_password'=>$this->login_password
		$methodData = http_build_query($this->__methodParams).'&login_password='.$this->login_password;
	...
	private function __getUrl()
	{
		...
		return sprintf('https://%s/site/%s?', $this->host, $this->__servlet);
	}
	 ...
	 * from the API.
	 */
	public function call($servletMethod, $params)
	{
		$this->__servlet = $servletMethod;
		//array_shift(explode('_', $servletMethod));
		$this->__method  = $servletMethod;
		//array_shift(explode('_', $servletMethod));
		//array_pop(explode('_', $servletMethod));
		//http_build_query($servletMethod);
		if ($params != '') { $this->__methodParams = $params; };
